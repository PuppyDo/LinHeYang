...
public class MyPanel01 extends JPanel implements KeyListener, Runnable {
    //定义我的坦克
    Hero01 hero = null;
    //定义敌人坦克
    Vector<EnemyTank01> enemyTanks = new Vector<>();
    //用于存放炸弹
    //当子弹击中坦克时，就加入一个bomb 对象
    Vector<Bomb01> bombs = new Vector<>();
    //定义三种图片，用于显示炸弹
    Image img1 = null;
    Image img2 = null;
    Image img3 = null;
        ...
        switch (type) {
            case "1":
                ...
                for (int i = 0; i < enemyTankSize; i++) {
                    EnemyTank01 enemyTank = new EnemyTank01(100 * (i + 1), 0);
            ...
            case "2": //继续上局游戏
                ...
                for (int i = 0;i < nodes.size();i++){
                    ...
                    EnemyTank01 enemyTank = new EnemyTank01(node.getX(),node.getY());
    ...
    public void paint(Graphics g) {
        ...
        for (int i = 0; i < enemyTanks.size(); i++) {
            EnemyTank01 enemyTank = enemyTanks.get(i);
        ...
        for (int i = 0; i < bombs.size(); i++) {
            Bomb01 bomb = bombs.get(i);
            // 根据当前炸弹的 life 值绘制对应的爆炸图片
            if (bomb.isLive()) {
                switch (bomb.getLife()) {
                    case 3:
...
//            }
        ...
        switch (tank.getDirection()) {
            ...
            case 2://坦克向下
                if (s.getX() > tank.getX() && s.getX() < tank.getX() + 40 &&
                    ...
                    if (!(tank instanceof Hero01)) {
                        Recorder.addlEnemyTankTotal();
                    }
                    //创建bomb对象，加入到bombs集合
                    Bomb01 bomb = new Bomb01(tank.getX(), tank.getY());
                    bombs.add(bomb);
                }
                break;
            case 1://坦克向右
    ...
    public void updateBombs() {
        Iterator<Bomb01> iterator = bombs.iterator();
        while (iterator.hasNext()) {
            Bomb01 bomb = iterator.next();
            // 让这个炸弹的生命值减少
            bomb.lifeDown();
            // 如果 bomb 生命值为 0，就从集合中删除
            if (bomb.getLife() == 0) {
                iterator.remove();
            }
        }
